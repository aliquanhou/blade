    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-lg shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input type="password" value={settings.apiKey} onChange={e => setSettings({...settings, apiKey: e.target.value})}
              placeholder={currentSettings.hasApiKey ? '•••••••• (set)' : 'Enter API key...'}
              className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-sm" />
          </div>
          <div>
        ...
        <div className="flex justify-end gap-2 px-5 py-3 border-t border-gray-700">
          <button onClick={onClose} className="px-4 py-2 text-sm border border-gray-600 rounded-lg hover:bg-gray-800">Cancel</button>
          <button onClick={handleSave} className="px-4 py-2 text-sm bg-blue-600 rounded-lg hover:bg-blue-700">Save</button>
        </div>
      </div>
    </div>
  );
